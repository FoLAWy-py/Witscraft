...
Every accepted or manually refreshed embedding is bound to a normalized SHA-256 content hash, provider-qualified model identifier, vector dimensions, operator-controlled `EMBEDDING_VERSION`, and UTC generation time. Semantic ranking compares a query only with memories whose model, dimensions, and version all match. Migration `0014` labels existing vectors `legacy:unversioned` / `legacy-v0`; migration `0017` adds authoritative `vector(1024)` storage and moves only dimension-compatible JSONB values. Other legacy dimensions remain available to structured importance and recency ranking without being silently compared to the current vector space or triggering automatic re-embedding.
