...
Narrative output allowance is separate from chapter pacing. The `normal_chat` purpose currently permits up to 8,192 output tokens, while conservative per-request budgeting leaves room for prompt and provider limits. The story's 500–5,000 language-aware chapter setting is only a cumulative minimum before ordinary chapter closure; it does not resize, pad, trim, or accept or reject an individual response. Explicit machine-readable impossible actions are rejected locally before quota preflight, and Continue changes the authoring constraint without adding an auxiliary call by itself.

An ordinary player-action turn adds one mandatory OpenAI `gpt-5.5` agency-edit call after authoring; it is not charged for **Continue**. The editor is fail-closed, audited as `consistency_check`, and included in the shared weekly percentage. Provider completion metadata is authoritative: a length-limited response may receive exactly one bounded same-scene continuation, then exact overlap is removed before persistence. A second incomplete result, or an interrupted or failed completion, fails closed. The per-turn external-call ceiling remains authoritative across authoring, truncation continuation, agency editing, state extraction, choices, consistency correction, and roadmap revision.
