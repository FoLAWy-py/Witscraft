...
Reference-style profiling is deterministic and local. A new normalized content hash creates one abstract profile and non-reproduction signature; a version-compatible re-import is a database cache hit. Neither path calls an LLM or embedding provider, so importing the same reference cannot consume weekly model allowance. Real-provider traffic is reserved for styled generation and the separately capped evaluation capture.

The versioned interactive-fiction evaluation capture has an independent hard ceiling of eight sequential provider calls and requires an explicit live-provider confirmation flag. Its approved sanitized output is reused by CI and release preflight without network access. A recapture is required only when the bound corpus, prompt versions, provider/model route, style-analysis version, or generation post-processing contract changes; routine test runs never repeat profiling, embedding, authoring, or judging traffic.

Narrative output allowance scales with the configured chapter target: the conservative request uses 1.15 tokens per requested CJK character or 1.6 tokens per requested whitespace-delimited word, plus a small structural reserve, and never exceeds the selected purpose/model hard ceiling. The engine measures returned prose in the configured user-facing unit. A result below the 85% acceptance floor may trigger up to two non-streaming replacement requests with the same route, output cap, established events, and player-agency constraint; each attempt starts from the best draft, and reaching the floor stops correction immediately. An acceptable primary result adds no call, while a failed or non-improving replacement keeps the best successful draft. These bounded attempts remain inside the account quota and per-turn external-call ceiling. Real-provider chapter-length scoring remains a release evaluation requirement. Explicit machine-readable impossible actions are rejected locally before quota preflight, and Continue changes the authoring constraint without adding an auxiliary call by itself.

An ordinary player-action turn adds one mandatory OpenAI `gpt-5.5` agency-edit call after authoring; it is not charged for **Continue**. The editor is fail-closed, audited as `consistency_check`, and included in the shared weekly percentage. If its measured replacement exceeds 115% of the configured chapter target, exactly one additional low-reasoning trim call is allowed; a result already inside the range adds no trim call. The per-turn external-call ceiling remains authoritative across authoring, expansion, agency editing, trimming, state extraction, choices, consistency correction, and roadmap revision.
