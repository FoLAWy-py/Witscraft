...
- Frontend workspace feature modules compile inside the existing narrow client boundary. TypeScript pins the settings and library-rail props plus purpose/model-role maps, ESLint rejects unused or missing imports after extraction, and Chromium verifies the story-owned world, protagonist roster, and rendered narrative-author, extraction, continuity, summary, and deployment-managed embedding roles through unchanged interactions.
