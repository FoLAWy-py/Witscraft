...
- Frontend workspace feature modules compile inside the existing narrow client boundary. TypeScript pins the settings props and purpose/model-role maps, ESLint rejects unused or missing imports after extraction, and Chromium verifies the rendered narrative-author, extraction, continuity, summary, and deployment-managed embedding roles through the unchanged settings interaction.
