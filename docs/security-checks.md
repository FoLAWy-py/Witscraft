...
- Ruff checks the API, tests, Alembic migration source, and production Python utilities including the memory embedding worker.
...
- Playwright runs a real Chromium session through login, workspace loading, player input, SSE parsing, narrative rendering, post-generation state synchronization, chapter configuration, and the default-closed roadmap spoiler disclosure. API responses are deterministic browser fixtures; PostgreSQL behavior is covered separately by the authenticated API journey.
