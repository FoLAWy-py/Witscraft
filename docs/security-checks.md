...
- Workspace-router boundary tests pin the onboarding paths, HTTP methods, and OpenAPI operation identifiers after internal domain-module extraction. PostgreSQL API journeys continue to own authorization, transaction, and response-contract evidence; a module move cannot silently rename or remove the browser contract.
