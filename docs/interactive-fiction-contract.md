...
The approved 2026-08-21 v3 capture used a public-domain excerpt from [Project Gutenberg eBook 1342](https://www.gutenberg.org/ebooks/1342), DeepInfra `Qwen/Qwen3-Max` for authoring, OpenAI `gpt-5.5` for the mandatory agency edit and conditional length trim, and a separate OpenAI `gpt-5.5` judge call. The bounded end-to-end run used seven of eight permitted provider calls, reused the deterministic style profile with zero profiling calls, produced 551 words for a 500-word target, and scored 96 player agency, 86 narrative quality, 90 roadmap-length adherence, 88 world/canon consistency, 72 abstract-profile adherence, and 85 overall. The capture is bound to deployment revision `6de512e9c435247aeac1c0af7b8061bd806e47de`. This single-case capture is release evidence, not a general benchmark; a route, prompt, corpus, style-analysis, or post-processing contract change requires a new bounded capture. CI replays the reviewed capture without network traffic. Preceding overlength and omitted-authorized-action failures remain regression evidence rather than being hidden.
