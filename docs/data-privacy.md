...
An authenticated user can download a JSON export from `GET /api/auth/export` or the Data and account section in Settings. The export contains the account profile, session metadata, worlds, characters, stories, branches, messages, generation records, plot events, state snapshots, summaries, canon facts, memories, preferences, model routes, and sanitized model-call metadata.
