...
from app.services.story_knowledge_repository import PreparedMemory, StoryKnowledgeRepository
from app.services.story_memory_retrieval import (
...
class StoryEngine:
    def __init__(self, llm_gateway: LLMGateway, session: AsyncSession, user_id: UUID):
        ...
        self.knowledge_repository = StoryKnowledgeRepository(
            session,
            embedding_task_max_attempts=(
                llm_gateway.settings.memory_embedding_task_max_attempts
            ),
        )
        self.turn_context = TurnContext()
        self.memory_retrieval_service = StoryMemoryRetrievalService(
            session,
    ...
    async def _get_story(self, story_id: UUID) -> Story:
        return await self.context_repository.get_story(story_id)
    ...
    def _memory_retrieval_service(self) -> StoryMemoryRetrievalService:
        service = getattr(self, "memory_retrieval_service", None)
        if service is None:
            service = StoryMemoryRetrievalService(
                self.session,
                self.embedding_service,
                self.turn_context,
            )
    ...
    async def _load_memories(
        ...
        return await self._memory_retrieval_service().load_memories(
            story_id,
            branch_id,
            query,
        )
    ...
    async def _load_database_semantic_scores(
        ...
        return await self._memory_retrieval_service().load_database_semantic_scores(
            story_id,
            branch_id,
            memories,
            query_embedding,
    ...
    async def _embed_query_once(self, query: str) -> list[float]:
        return await self._memory_retrieval_service().embed_query_once(query)
    async def _load_canon_facts(self, story_id: UUID, branch_id: UUID) -> list[str]:
        return await self.context_repository.load_canon_facts(story_id, branch_id)
    ...
    async def _prepare_extracted_knowledge(
        ...
        *,
        ...
        prepared = await self.knowledge_repository.prepare_extracted_knowledge(
            story,
            branch,
            memories,
            canon_facts,
    ...
    async def _load_recent_memory_candidates(
        ...
        return await self.knowledge_repository.load_recent_memory_candidates(
            story_id, branch_id
        )
    ...
    def _normalize_memory_text(value: str) -> str:
        return StoryKnowledgeRepository.normalize_memory_text(value)

    @staticmethod
    def _memory_entity_tags(content: str, entity_names: list[str]) -> tuple[str, ...]:
        return StoryKnowledgeRepository.memory_entity_tags(content, entity_names)

    @staticmethod
    def _memory_importance(content: str, entity_tags: tuple[str, ...]) -> int:
        return StoryKnowledgeRepository.memory_importance(content, entity_tags)
    ...
    def _memory_is_near_duplicate(
        ...
        return StoryKnowledgeRepository.memory_is_near_duplicate(
            content,
            entity_tags,
            existing_content,
            existing_entities,
    ...
    def _refresh_duplicate_memory(
        ...
        StoryKnowledgeRepository.refresh_duplicate_memory(
            existing, importance, entity_tags
        )
    ...
    def _add_prepared_memories(
        ...
        return self.knowledge_repository.add_prepared_memories(
            story, branch, source_message_id, memories, source
        )

    @staticmethod
    def _merge_memory_results(existing: list[str], added: list[str]) -> list[str]:
        return StoryKnowledgeRepository.merge_memory_results(existing, added)
    ...
    def _add_prepared_canon_facts(
        ...
        self.knowledge_repository.add_prepared_canon_facts(
            story, branch, source_message_id, facts, source
        )

    async def _memory_exists(self, story_id: UUID, branch_id: UUID, content: str) -> bool:
        return await self.knowledge_repository.memory_exists(story_id, branch_id, content)

    async def _canon_fact_exists(self, story_id: UUID, branch_id: UUID, content: str) -> bool:
        return await self.knowledge_repository.canon_fact_exists(
            story_id, branch_id, content
        )

    async def _load_world_context(self, story: Story) -> dict:
        return await self.context_repository.load_world_context(story)

    async def _load_character_context(self, story: Story) -> dict:
        return await self.context_repository.load_character_context(story)

    async def _load_user_preferences(self, user_id: UUID) -> list[str]:
        return await self.context_repository.load_user_preferences()
    ...
    async def _load_recent_messages(
        ...
        return await self.context_repository.load_recent_messages(
            story_id, branch_id, exclude_message_ids, after_message_id
        )

    async def _load_latest_summary(self, story_id: UUID, branch_id: UUID) -> StorySummary | None:
        return await self.context_repository.load_latest_summary(story_id, branch_id)

    async def _main_character_name(self, story: Story) -> str | None:
        return await self.context_repository.main_character_name(story)
