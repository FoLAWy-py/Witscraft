...
write_runtime_env() {
  ...
  database_password="$(openssl rand -hex 24)"
  {
    ...
    print "STAGING_DATABASE_BACKEND_PORT=${WITSCRAFT_STAGING_DATABASE_BACKEND_PORT:-15433}"
...
load_runtime_env() {
  ...
  if [[ -z "${STAGING_DATABASE_BACKEND_PORT:-}" ]]; then
    STAGING_DATABASE_BACKEND_PORT=15433
    print "STAGING_DATABASE_BACKEND_PORT=$STAGING_DATABASE_BACKEND_PORT" >> "$RUNTIME_ENV"
    chmod 600 "$RUNTIME_ENV"
    export STAGING_DATABASE_BACKEND_PORT
  fi
  ...
  : "${STAGING_DATABASE_BACKEND_PORT:?STAGING_DATABASE_BACKEND_PORT is required}"
  ...
  validate_port STAGING_DATABASE_BACKEND_PORT "$STAGING_DATABASE_BACKEND_PORT"
  [[ "$STAGING_DATABASE_PORT" != "$STAGING_DATABASE_BACKEND_PORT" ]] || \
    fail "database gateway and backend ports must differ"
}
...
prepare() {
  ...
  sed \
    ...
    -e "s/__STAGING_DATABASE_PORT__/$STAGING_DATABASE_PORT/g" \
    -e "s/__STAGING_DATABASE_BACKEND_PORT__/$STAGING_DATABASE_BACKEND_PORT/g" \
...
configure_runtime_environment() {
  ...
  export DATABASE_PORT="$STAGING_DATABASE_BACKEND_PORT"
...
write_staging_secrets() {
  ...
  cp "$SOURCE_SECRETS" "$temporary_secrets"
  {
    ...
    print "DATABASE_PORT=$STAGING_DATABASE_BACKEND_PORT"
    print "DATABASE_NAME=$DATABASE_NAME"
    print "DATABASE_SSL_MODE=require"
