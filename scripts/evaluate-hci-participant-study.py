...
RUNTIME_SESSIONS = RUNTIME / "hci" / "sessions"
...
def _session_path(participant_code: str, path: Path) -> Path:
    _require(
        re.fullmatch(r"P-[A-F0-9]{8}", participant_code) is not None,
        "participant code must be generated by this tool",
    )
    resolved = _runtime_path(path)
    expected = (RUNTIME_SESSIONS / f"{participant_code}.json").resolve()
    _require(resolved == expected, "session filename must match its anonymous participant code")
    return resolved
...
def _session_template(
    ...
    session = {
        ...
        "first_scene_seconds": None,
        "normal_action_agency_correct": None,
        "continue_scope_correct": None,
    ...
    _validate_session(session, 0)
    return session


def _require_completed_session(session: dict[str, Any]) -> None:
    if not session["valid"]:
        return
    participant_code = session["participant_code"]
    _require(
        all(task["status"] != "not_attempted" for task in session["tasks"].values()),
        f"session {participant_code} has unattempted tasks",
    )
    _require(session["first_scene_seconds"] is not None, f"session {participant_code} lacks first-scene time")
    _require(
        session["normal_action_agency_correct"] is not None,
        f"session {participant_code} lacks normal-action authority result",
    )
    _require(
        session["continue_scope_correct"] is not None,
        f"session {participant_code} lacks Continue authority result",
    )
    _require(
        session["recovery_attempted"] and session["recovery_succeeded"] is not None,
        f"session {participant_code} lacks recovery result",
    )
    _require(session["confidence"] is not None, f"session {participant_code} lacks confidence")
    if session["sus_responses"] is None:
        _require(
            "questionnaire_incomplete" in session["protocol_deviations"],
            f"session {participant_code} must mark an incomplete questionnaire",
        )


def append_session(study: dict[str, Any], session: dict[str, Any]) -> dict[str, Any]:
    validate_study(study)
    _validate_session(session, len(study["sessions"]))
    _require_completed_session(session)
    participant_code = session["participant_code"]
    _require(
        all(existing["participant_code"] != participant_code for existing in study["sessions"]),
        "participant code already exists in the study",
    )
    updated = {**study, "sessions": [*study["sessions"], session]}
...
def _parse_args() -> argparse.Namespace:
    ...
    template = subparsers.add_parser(
        "session-template", help="create one private, incomplete facilitator session form"
    )
    template.add_argument("--output", type=Path, required=True)
    template.add_argument("--participant-code", required=True)
    template.add_argument("--session-date", required=True)
    template.add_argument("--device-class", choices=sorted(DEVICE_CLASSES), required=True)
    template.add_argument("--viewport-class", choices=sorted(VIEWPORT_CLASSES), required=True)
    template.add_argument("--experience-band", choices=sorted(EXPERIENCE_BANDS), required=True)
    template.add_argument("--confirm-adult", action="store_true", required=True)
    template.add_argument("--confirm-target-reader", action="store_true", required=True)
    template.add_argument("--confirm-non-contributor", action="store_true", required=True)
    template.add_argument("--confirm-task-script-unexposed", action="store_true", required=True)
    template.add_argument("--confirm-observation-consent", action="store_true", required=True)
    append = subparsers.add_parser(
        "append-session", help="validate and atomically append one completed session"
    )
    append.add_argument("--study", type=Path, required=True)
    append.add_argument("--session", type=Path, required=True)
    append.add_argument("--confirm-consume-session-file", action="store_true", required=True)
...
def main() -> None:
    ...
    try:
        ...
        if args.command == "session-template":
            output = _session_path(args.participant_code, args.output)
            _require(not output.exists(), "private session output already exists")
            session = _session_template(
                args.participant_code,
                args.session_date,
                args.device_class,
                args.viewport_class,
                args.experience_band,
            )
            _write_private_json(output, session)
            print(f"Private incomplete HCI session initialized: {output}")
            return
        if args.command == "append-session":
            study_path = _runtime_path(args.study)
            _require(study_path.is_file(), "private study input is missing")
            _require(study_path.stat().st_mode & 0o777 == 0o600, "private study must have mode 0600")
            unbound_session_path = _runtime_path(args.session)
            _require(unbound_session_path.is_file(), "private session input is missing")
            session_payload = json.loads(unbound_session_path.read_text(encoding="utf-8"))
            _require(isinstance(session_payload, dict), "private session must be an object")
            session_path = _session_path(
                session_payload.get("participant_code", ""), unbound_session_path
            )
            _require(session_path.stat().st_mode & 0o777 == 0o600, "private session must have mode 0600")
            study_payload = json.loads(study_path.read_text(encoding="utf-8"))
            updated = append_session(study_payload, session_payload)
            _write_private_json(study_path, updated)
            session_path.unlink()
            print("Completed anonymous session appended; temporary session file deleted")
            return
