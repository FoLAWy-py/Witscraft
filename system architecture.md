...
The browser workspace retains one client-side orchestration boundary for authenticated state, URL synchronization, and mutations, while feature UI is colocated in private workspace modules. Account security, quota, preferences, provider health, and purpose routing render through `SettingsView`; their bilingual labels and typed model-role configuration live in `workspace-config.ts`. The story library, current story-owned world, and character roster render through the controlled `LeftRail` view in `library-rail.tsx`. Current scene state, branch controls, relationship and inventory records, summaries, export controls, editable knowledge, and model-call telemetry render through `RightInspector` in `right-inspector.tsx`. The inspector owns only branch-local progressive-disclosure counts and resets them when the active branch changes; the orchestrator remains the sole owner of mutations, API calls, concurrency guards, and durable state. It also passes only the current world: unreachable client state and handlers for creating, switching, or deleting alternate worlds were removed to match the one-world-per-story product invariant, while current-world editing remains available. Moving these modules does not add a server boundary or duplicate durable state: Next.js includes them in the existing client module graph, and all credentials remain server-managed behind the API.
